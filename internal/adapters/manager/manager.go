...
import (
	"bytes"
	"dev.rubentxu.devops-platform/adapters/grpc/node"
	"dev.rubentxu.devops-platform/adapters/grpc/scheduler"
...
func (m *Manager) SelectWorker(t task.Task) (*node.Node, error) {
	...
	if candidates == nil {
		msg := fmt.Sprintf("No available candidates match resource request for task %v", t.Metadata.ID)
		err := errors.New(msg)
		return nil, err
	}
...
func (m *Manager) updateTasks() {
	for _, worker := range m.Workers {
		...
		for _, t := range tasks {
			log.Printf("[manager] Attempting to update task %v", t.Metadata.ID)

			result, err := m.TaskDb.Get(t.Metadata.ID.String())
			if err != nil {
			...
			if taskPersisted.Status.State != t.Status.State {
				taskPersisted.Status.State = t.Status.State
			}

			taskPersisted.Status.StartTime = t.Status.StartTime
			taskPersisted.Status.FinishTime = t.Status.FinishTime
			taskPersisted.Status.ContainerID = t.Status.ContainerID
			//taskPersisted.Status.HostPorts = t.HostPorts

			m.TaskDb.Put(taskPersisted.Metadata.ID.String(), taskPersisted)
		}
	}
}
...
func (m *Manager) doHealthChecks() {
	tasks := m.GetTasks()
	for _, t := range tasks {
		if t.Status.State == task.Running && t.Status.RestartCount < 3 {
			err := m.checkTaskHealth(*t)
			if err != nil {
				if t.Status.RestartCount < 3 {
					m.restartTask(t)
				}
			}
		} else if t.Status.State == task.Failed && t.Status.RestartCount < 3 {
			m.restartTask(t)
		}
	}
}
